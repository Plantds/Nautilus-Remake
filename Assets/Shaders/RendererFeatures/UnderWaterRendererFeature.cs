...
public sealed class UnderWaterRendererFeature : ScriptableRendererFeature
{
    ...
    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        ...
        UnderWaterVolumeComponent myVolume = VolumeManager.instance.stack?.GetComponent<UnderWaterVolumeComponent>();
    ...
    private class CustomPostRenderPass : ScriptableRenderPass
    {
        ...
        private static void ExecuteMainPass(RasterCommandBuffer cmd, RTHandle sourceTexture, Material material)
        {
            ...
            if (myVolume != null)
            {
                ...
                s_SharedPropertyBlock.SetFloat("_Depth_Intensity", myVolume.depth.value);
                s_SharedPropertyBlock.SetColor("_Depth_Color", myVolume.depthcolor.value);
                s_SharedPropertyBlock.SetFloat("_Near", myVolume.near.value);
                s_SharedPropertyBlock.SetFloat("_Far", myVolume.far.value);


            }
            else
            {
                s_SharedPropertyBlock.SetFloat("_Intensity", 0.0f);
            }
